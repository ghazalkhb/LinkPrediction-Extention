...
def link_prediction():
    emb = KeyedVectors.load_word2vec_format("Output/samplen.emb", binary=False)
    # x_train and y_train are the files containing training data
    x_train = edge_embedding_comm(emb, load_data_comm("Input/new/trainx.txt"))
    y_train = np.loadtxt("Input/new/trainy.txt")
    ...
    x_test = edge_embedding_comm(emb, load_data_comm("Input/new/testx.txt"))
    y_test = np.loadtxt("Input/new/testy.txt")
