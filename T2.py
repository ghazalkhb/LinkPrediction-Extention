...
def create_time_windows(dataframe, window_size, max_time):
    return [
        dataframe[(dataframe['timestamp'] >= start_time) & (dataframe['timestamp'] < start_time + window_size)]
...
def create_graph(dataframe):
    edge_index = torch.tensor(
        np.vstack([dataframe['um_encoded'].values, dataframe['dm_encoded'].values]), dtype=torch.long
    )
...
class GATTransformerModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.gat(x, data.edge_index)  # Apply GAT
        x = self.transformer_encoder(x)  # Apply Transformer
        return self.fc(x)

model = GATTransformerModel(num_nodes=num_nodes).to(device)
...
def advanced_negative_sampling(edge_index, num_nodes, existing_edges, alpha=0.1):
    ...
    for _ in range(num_edges):
        while True:
            src, dest = torch.multinomial(degree_prob, 1).item(), torch.randint(0, num_nodes, (1,)).item()
...
def train(model, data):
    ...
    optimizer.zero_grad()
    out = model(data)
    pos_pred = torch.sigmoid((out[data.edge_index[0]] * out[data.edge_index[1]]).sum(dim=1))
...
for epoch in range(200):
    for i, graph in enumerate(train_graphs):
        ...
        print(f'Epoch {epoch}, Training Window {i}, Loss: {loss:.4f}')

def evaluate(model, data):
    model.eval()
    out = model(data)
    pos_pred = torch.sigmoid((out[data.edge_index[0]] * out[data.edge_index[1]]).sum(dim=1)).detach().cpu().numpy()
    neg_edge_index = advanced_negative_sampling(data.edge_index, data.num_nodes, data.edge_index)
    neg_pred = torch.sigmoid((out[neg_edge_index[0]] * out[neg_edge_index[1]]).sum(dim=1)).detach().cpu().numpy()
