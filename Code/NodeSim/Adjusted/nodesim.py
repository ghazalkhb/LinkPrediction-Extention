...
class Graph():
    ...
    def nodesim_walk(self, walk_length, start_node):
        ...
        while len(walk) < walk_length:
            cur = walk[-1]
            if cur not in neighbors or not neighbors[cur]:
                print(f"Node {cur} has no neighbors, ending walk.")
                break
            if cur not in probabilities or not probabilities[cur]:
                print(f"Node {cur} has no probabilities, ending walk.")
                break

            nextnode = random.choices(neighbors[cur], probabilities[cur])[0]
    ...
    def simulate_walks(self, num_walks, walk_length):
        ...
        for walk_iter in range(num_walks):
            ...
            for node in nodes:
                walk = self.nodesim_walk(walk_length=walk_length, start_node=node)
                if walk:  # Ensure walk is not empty before adding
                    walks.append(walk)
        return walks
